...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    layer3, cache3 = affine_forward(layer2, self.params['W2'], self.params['b2'])
    scores = layer3
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dlayer3, dW2, db2 = affine_backward(dloss, cache3)
    dlayer2 = relu_backward(dlayer3, cache2)
    dx, dW1, db1 = affine_backward(dlayer2, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(self.num_layers):
        self.params['W' + str(i+1)] = np.random.normal(loc=0.0, scale=weight_scale, size=[layer_dims[i], layer_dims[i+1]])
  ...
  def loss(self, X, y=None):
    ...
    # layer, etc.                                                              #
    ############################################################################
    affine_caches, bn_caches, relu_caches, dropout_caches = [], [], [], []
    x = X
    for i in xrange(self.num_layers):
        ...
        if i < self.num_layers-1:
            if self.use_dropout:
                affine_out, dropout_cache = dropout_forward(affine_out, self.dropout_param)
    ...
    for i in xrange(self.num_layers-1, -1, -1):
        if i == self.num_layers - 1:
            affine_cache = affine_caches[i]
            dx, dw, db = affine_backward(dout, affine_cache)
        else:
            relu_cache = relu_caches[i]
            affine_cache = affine_caches[i]
            # relu backward
        ...
        dout = dx  # feed into next iteration
